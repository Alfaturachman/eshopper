...
class Checkout extends CI_Controller {
	...
	public function customer_login(){
		...
		$cus_pass = $this->input->post('cus_password',true);
		$user_details = $this->CheckoutModel->get_user_login_by_email($cus_email);
		if ($user_details !== NULL && $this->CheckoutModel->verify_customer_password($user_details, $cus_pass)){
	...
	public function shipping(){
			if ($this->session->userdata("cus_id") === NULL) {
				redirect("checkout");
			}
	...
	public function place_order(){
		...
		if ($this->session->userdata('cus_id') === NULL || $this->session->userdata('shipping_id') === NULL) {
			$this->session->set_flashdata("flash_msg","<font class='btn-warning alert alert-danger'>Please complete the billing & shipping steps first.</font>");
			redirect("checkout");
		}
		if ($this->cart->total_items() <= 0) {
			$this->session->set_flashdata("flash_msg","<font class='btn-warning alert alert-danger'>Your cart is empty.</font>");
			redirect("products");
		}
		if ( ! $this->CheckoutModel->validate_stock()) {
			$this->session->set_flashdata("flash_msg","<font class='btn-warning alert alert-danger'>Sorry, one or more items are out of stock.</font>");
			redirect("payment");
		}

		$payment_method = $this->input->post('payment_gateway',true);
		if($payment_method!=NUll){
			$totals = $this->CheckoutModel->calculate_order_total();
			$this->session->set_userdata('g_total', $totals['total']);
