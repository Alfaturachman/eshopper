...
// Start pagination
		...
		$data['post_by_brand_id'] = $this->HomeModel->get_all_product_pagination($config['per_page'],$this->uri->segment(3));
...
// End pagination
	...
	public function show_product_by_price_range(){
		$data = array();
		 $min_range = (float) $this->input->post('amount1');
		 $max_range = (float) $this->input->post('amount2');
		$data['slider'] = $this->load->view('front/advertise_top','',true);
		$data['recommended'] = "";
		$data['category_brand'] = $this->load->view('front/category','',true);
	...
	public function insert_contact_info(){
			$this->form_validation->set_rules('contact_name', 'Name', 'trim|required|min_length[2]|max_length[100]');
			$this->form_validation->set_rules('contact_email', 'Email', 'trim|required|valid_email');
			$this->form_validation->set_rules('contact_subject', 'Subject', 'trim|required|max_length[200]');
			$this->form_validation->set_rules('contact_message', 'Message', 'trim|required|min_length[10]|max_length[2000]');
