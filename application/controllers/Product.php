...
class Product extends CI_Controller{
	...
	public function update_product(){
		...
		if($_FILES['pro_image']['name']=="" || $_FILES['pro_image']['size']==""){
			...
			$old = $this->ProductModel->edit_product_model($product_id);
			$this->ProductModel->update_product_model($product_image);
			if ($old !== NULL && $this->is_safe_upload_path($old->pro_image)) {
				@unlink(FCPATH . $old->pro_image);
			}
			$this->session->set_flashdata("update_pro_msg","Product Updated Successfully");
			redirect('edit-product/'.$product_id);
		}
	...
	private function is_safe_upload_path($path){
		...
		$real_uploads = realpath(FCPATH . 'uploads');
		$real_file = realpath(FCPATH . $path);
		if ($real_uploads === FALSE || $real_file === FALSE) {
			return FALSE;
		}
		return strpos($real_file, $real_uploads . DIRECTORY_SEPARATOR) === 0;
	}
