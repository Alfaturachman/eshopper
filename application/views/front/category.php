			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<h4 class="panel-title">
						<a data-toggle="collapse" data-parent="#accordian" href="#<?php echo html_escape($maincat->category_name);?>">
							...
							<?php echo html_escape($maincat->category_name)?>	
						</a>
					</h4>
				</div>
				<div id="<?php echo html_escape($maincat->category_name);?>" class="panel-collapse collapse">
					<div class="panel-body">
						<ul>
							<?php foreach ($all_sub_category as  $subcat) {?>
								<?php if($subcat->category_sub_id == $maincat->category_id){?>
										<li><a href="<?php echo base_url()?>show-post-by-sub-cat-id/<?php echo $subcat->sub_cat_id?>"><?php echo html_escape($subcat->sub_category_name)?></a></li>
								<?php }?>
							<?php } ?>						
						</ul>
					</div>
				...
				<ul class="nav nav-pills nav-stacked">
					...
					<li><a href="<?php echo base_url()?>show-post-by-brand-id/<?php echo $brand->brand_id?>"> <span class="pull-right">
						<?php
						 ...
						 ?>
						 <?php echo "(" . html_escape($total_brand) . ")"?>
					</span><?php echo html_escape($brand->brand_name);?></a></li>
					<?php } ?>
				</ul>
			</div>
		</div><!--/brands_products-->
